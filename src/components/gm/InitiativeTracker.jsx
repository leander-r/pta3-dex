...
import { getActualStats, calculatePokemonHP, calcModifier } from '../../utils/dataUtils.js';
...
const AddCustom = ({ onAdd }) => {
    ...
    const submit = () => {
        ...
        onAdd({
            ...
            initiative: initiative === '' ? null : parseInt(initiative) || 0,
            spd: 3, hp: { current: hpMax, max: hpMax },
            stats: null, types: [], moves: [], features: [], avatar: ''
        });
        setName(''); setHpMax(20); setInitiative('');
    };
    ...
    return (
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center', padding: '10px', background: 'var(--bg-secondary)', borderRadius: '8px', border: '1px solid var(--border-medium)' }}>
            ...
            <input type="number" placeholder="Initiative" value={initiative} onChange={(e) => setInitiative(e.target.value)}
                style={{ width: '90px', padding: '6px 10px', borderRadius: '6px', border: '1px solid var(--border-medium)', background: 'var(--input-bg)', color: 'var(--text-primary)', fontSize: '13px' }} />
...
const CombatantRow = ({ c, isActive, onRoll, onHpChange, onRemove }) => (
    <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            ...
            <button onClick={onRoll} title="Roll initiative (d20 + Spd mod)"
...
const InitiativeTracker = () => {
    ...
    const rollOne = (id) => {
        setCombatants(prev => prev.map(c => {
            ...
            return { ...c, initiative: roll + calcModifier(c.spd) };
        }));
    };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div className="section-card-purple">
                ...
                <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '14px' }}>
                    Session scratchpad for turn order. Add combatants from your NPC Roster, player trainers, or a one-off custom entry.
                </p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', marginBottom: '10px', padding: '10px 14px', borderRadius: '8px', background: 'var(--input-bg)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={rollAll} disabled={combatants.length === 0}
